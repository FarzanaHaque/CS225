...
idx=(idx+probeDistance) %size;
//if(idx==start) break; //how could this happen?????
}
++elems;
table[idx]=new pair<K,V>;
...
/*std::pair<K, V>** table2;
    table2 = new pair<K, V>*[size];
    should_probe = new bool[size];
    for (size_t i = 0; i < other.size; i++) {
        should_probe2[i] = other.should_probe[i];
        if (other.table[i] == NULL ||i=index)
            table[i] = NULL;
        else
